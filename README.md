...
<head>
  ...
  <title>ಗಣೇಶ ಯಕ್ಷಗಾನ - ಸ್ಕ್ರಾಚ್ & ಗೆಲ್ಲಿ (Yakshagana Theme)</title>
...
<body>
  ...
  <div class="container">
    ...
    <section class="card-panel" id="loginPanel">
      ...
      <p class="muted">OTP ಕೆಲಸ ಮಾಡಲು ಈ ಪೇಜ್ ಅನ್ನು HTTPS ಅಥವಾ localhost ನಲ್ಲಿ ಓದಿರಿ. (Phone Auth requires a live domain)</p>

      <div class="notice">ನೋಟ್: Firebase Console → Authentication → Sign-in method → Phone ಅನ್ನು ಸಕ್ರಿಯಗೊಳಿಸಿ. ಮತ್ತು Console → Authentication → Authorized domains ಗೆ ನಿಮ್ಮ ಡೊಮೈನ್ ಸೇರಿಸಿ.</div>
    ...
    <section class="card-panel" id="gamePanel" style="display:none">
      ...
      <div id="cardsGrid" class="grid" aria-hidden="true"></div>
  ...
  <footer>📍 ಮುಳ್ಳೆರಿಯ | ಸಂಪರ್ಕ: 9876543210</footer>

<script>
/*
  Firebase v8 namespaced implementation (improved OTP handling)
...
*/
(function(){
  // guard: ensure firebase loaded
  if (typeof firebase === 'undefined') {
    const err = 'Firebase libraries failed to load. Ensure scripts are present and network is allowed.';
    document.getElementById('errorBox').style.display = 'block';
    document.getElementById('errorBox').textContent = err;
    console.error(err);
    return;
  }
  ...
  // ---------- improved reCAPTCHA helper ----------
  // returns widget id when ready
  function createRecaptcha() {
    return new Promise((resolve, reject) => {
      try{
        ...
        const renderResult = window.recaptchaVerifier.render();
        // renderResult may be a number or a promise depending on environment
        if (renderResult && typeof renderResult.then === 'function') {
          renderResult.then(id => resolve(id)).catch(err => reject(err));
        } else {
  ...
  sendOtpBtn.addEventListener('click', async ()=>{
    ...
    try{
      ...
      // create and render reCAPTCHA, wait for widget ready
      try{ await createRecaptcha(); } catch(recErr){
        console.error('recaptcha create failed', recErr);
        errorBox.style.display = 'block'; errorBox.textContent = 'reCAPTCHA ಪೂರ್ವಾಪರ ಕ್ರೀಡಿತವಾಗಲಿಲ್ಲ: ' + (recErr.message || recErr);
        loginMsg.textContent = '';
        return;
      }

      // signInWithPhoneNumber (compat v8)
      ...
      // show clear error to admin/user - common causes: Phone Auth not enabled, unauthorized domain, network
      let msg = err && err.message ? err.message : String(err);
      msg += ' — ಸಾಮಾನ್ಯ ಕಾರಣಗಳು: Phone Auth ನ್ನು ಸಕ್ರಿಯಪಡಿಸಿಲ್ಲ ಅಥವಾ ಡೊಮೈನ್ Firebase ನಲ್ಲಿ Authorized ಆಗಿಲ್ಲ.';
